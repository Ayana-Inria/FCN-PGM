    ...
    def __getitem__(self, i):
        ...
        if random_idx in self.label_cache_.keys():
            ...
            if self.ids_type == 'TRAIN':
                if self.gt_type == 'block':
                    label = np.asarray(sample_groundtruth(convert_from_color(io.imread(self.label_files[random_idx])), self.gt_modification), dtype='int64')
                elif self.gt_type == 'rand':
                    label = np.asarray(deteriorate_groundtruth(convert_from_color(io.imread(self.label_files[random_idx])), self.gt_modification), dtype='int64')
                elif self.gt_type == 'conncomp':
